...
class Auth::OtpProcessorTest < ActiveSupport::TestCase
  ...
  describe "#generate" do
    test "정상적으로 OTP를 생성하고 메일을 보낸다" do
      result = Auth::OtpProcessor.new(email: @user.email_address).generate(with_otp: true)
    ...
    test "이메일 형식이 잘못되면 실패한다" do
      result = Auth::OtpProcessor.new(email: "invalid_email").generate
      assert result.failure?
      assert_equal "유효한 이메일을 입력해 주세요.", result.error_message
    end

    test "존재하지 않는 사용자면 실패한다" do
      result = Auth::OtpProcessor.new(email: "ghost@example.com").generate
      assert result.failure?
  ...
  describe "#verify" do
    ...
    test "OTP가 일치하면 성공한다" do
      result = Auth::OtpProcessor.new(email: @user.email_address, otp: @otp).verify
      assert result.success?
    end

    test "OTP가 틀리면 실패한다" do
      result = Auth::OtpProcessor.new(email: @user.email_address, otp: "000000").verify
      assert result.failure?
      assert_equal "OTP가 일치하지 않습니다.", result.error_message
    end

    test "OTP 형식이 유효하지 않으면 실패한다" do
      result = Auth::OtpProcessor.new(email: @user.email_address, otp: "abc123").verify
      assert result.failure?
      assert_equal "OTP는 6자리 숫자여야 합니다.", result.error_message
    end

    test "존재하지 않는 사용자면 실패한다" do
      result = Auth::OtpProcessor.new(email: "ghost@example.com", otp: "123456").verify
      assert result.failure?
      assert_equal "사용자를 찾을 수 없습니다.", result.error_message
    end
  end
