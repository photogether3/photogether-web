...
class Auth::LoginUseCaseTest < ActiveSupport::TestCase
  ...
  test "로그인 성공 시 토큰을 반환한다" do
    result = Auth::LoginUseCase.new(email: @user.email_address, password: "Password1!").call
  ...
  test "이메일이 유효하지 않으면 실패한다" do
    result = Auth::LoginUseCase.new(email: "invalid_email", password: "Password1!").call
  ...
  test "등록되지 않은 이메일이면 실패한다" do
    result = Auth::LoginUseCase.new(email: "nonexist@example.com", password: "Password1!").call
  ...
  test "비밀번호가 없으면 실패한다" do
    result = Auth::LoginUseCase.new(email: @user.email_address, password: "").call
  ...
  test "비밀번호가 틀리면 실패한다" do
    result = Auth::LoginUseCase.new(email: @user.email_address, password: "WrongPassword!").call
