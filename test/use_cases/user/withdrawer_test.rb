...
class User::WithdrawerTest < ActiveSupport::TestCase
  setup do
    @password = "Password1!"
    @user = User.create!(
  ...
  test "OTP 형식이 잘못된 경우 실패한다" do
    result = User::Withdrawer.new(@user, { otp: "abc" }).call
  ...
  test "사용자가 없을 경우 실패한다" do
    result = User::Withdrawer.new(nil, { otp: @otp }).call
  ...
  test "OTP가 일치하지 않을 경우 실패한다" do
    result = User::Withdrawer.new(@user, { otp: "654321" }).call
  ...
  test "모든 조건이 충족되면 사용자 계정이 삭제된다" do
    user_id = @user.id
    result = User::Withdrawer.new(@user, { otp: @otp }).call
