class Api::V1::UserApiController < Api::ApplicationApiController
  ...
  def update_password_by_otp
    result = User::OtpPasswordUpdater.new(params).call
  ...
  def update_password
    result = User::CurrentPasswordUpdater.new(@current_user, params).call
    return render_result(result) if result.failure?
  ...
  def reset_data
    result = User::DataResetter.new(@current_user, params).call
