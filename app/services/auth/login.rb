class Auth::Login
  ...
  def call
    ...
    return Result.failure(err_msg) unless user
    return Result.failure(err_msg) if !user.authenticate(@password)
    return Result.failure("이메일 인증을 완료해주세요.") unless user.is_email_verified
