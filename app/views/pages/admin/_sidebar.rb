        ...
        ul(class: "flex flex-col gap-4") do
          li do
            a(href: @admin_dashboard_path,
              class: "flex flex-col gap-2 text-xs items-center p-2 rounded-md #{current_path?(@admin_dashboard_path) && 'bg-base-200' } ") do
              raw(<<~SVG.html_safe)
          ...
          li do
            a(href: @admin_users_path,
              class: "flex flex-col gap-2 text-xs items-center p-2 rounded-md #{current_path?(@admin_users_path) && 'bg-base-200' }") do
              raw(<<~SVG.html_safe)
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
          ...
          li do
            a(href: @admin_polices_path,
              class: "flex flex-col gap-2 text-xs items-center p-2 rounded-md #{current_path?(@admin_polices_path) && 'bg-base-200' }") do
              raw(<<~SVG.html_safe)
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
