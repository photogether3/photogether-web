class Views::Pages::Home::Index < Views::Base
  def view_template
    div(class: "motion-preset-fade-lg") do
      # 상단 피드백 알림 영역
      nav(class: "bg-secondary text-secondary-content hidden sm:block") do
        div(class: "pg-container") do
          div(class: "flex justify-center items-center h-full gap-2 py-3") do
            raw(<<~SVG.html_safe)
              <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="20" height="20" rx="10" transform="matrix(1 0 0 -1 6.5 26)" fill="#00AC69"/>
                <path d="M16.3992 13.441C17.062 13.441 17.5992 12.9038 17.5992 12.241C17.5992 11.5783 17.062 11.041 16.3992 11.041C15.7365 11.041 15.1992 11.5783 15.1992 12.241C15.1992 12.9038 15.7365 13.441 16.3992 13.441Z" fill="#1B1D21"/>
                <path d="M14.8992 14.4312C14.7888 14.4312 14.6992 14.5207 14.6992 14.6312V15.4312C14.6992 15.5416 14.7888 15.6312 14.8992 15.6312H15.6008C15.7112 15.6312 15.8008 15.7207 15.8008 15.8312V19.5938H14.7C14.5895 19.5938 14.5 19.6833 14.5 19.7937V20.5938C14.5 20.7042 14.5895 20.7937 14.7 20.7937H18.3C18.4105 20.7937 18.5 20.7042 18.5 20.5938V19.7937C18.5 19.6833 18.4105 19.5938 18.3 19.5938H17.6008V14.6312C17.6008 14.5207 17.5112 14.4312 17.4008 14.4312H14.8992Z" fill="#1B1D21"/>
              </svg>
            SVG
            p { "포토게더가 성장하는 중! 여러분의 의견이 필요합니다. " }
            a(class: "underline font-bold",
            href: "/not-yet", data: { turbo_method: "get", turbo_frame: "not_yet" }) { "앱 다운로드 후 피드백 남기기" }
          end
        end
      end
      ...
      footer do
        div(class: "bg-black py-[20px] lg:py-[48px] px-4") do
          div(class: "pg-container flex flex-col gap-[24px]") do
            div(class: "w-full flex flex-col gap-[16px] lg:flex-row lg:justify-between lg:items-center lg:gap-0") do
              render logo_icon

              div(class: "flex items-center gap-[24px]") do
                a(class: "text-xs lg:text-sm underline text-primary",
                href: "/not-yet", data: { turbo_method: "get", turbo_frame: "not_yet" }) { "앱 다운로드 후 피드백 남기기" }
                img(src: "/images/landing/footer/item01.png", class: "w-[24px] lg:w-[40px] object-contain")
              end
            end

            div(class: "border-b border-[#A6A6A6] w-full")

            div(class: "text-[#A3A3A3] flex flex-col gap-[16px] lg:flex-row lg:justify-between lg:items-center lg:gap-0") do
              div(class: "flex gap-[16px]") do
                a(href: "/not-yet", data: { turbo_method: "get", turbo_frame: "not_yet" }) { "이용약관" }
                a(class: "text-primary font-bold",
                href: "/not-yet", data: { turbo_method: "get", turbo_frame: "not_yet" }) { "개인정보처리방침" }
              end

              div(class: "text-sm flex flex-col gap-[8px] lg:flex-row lg:gap-[32px]") do
                ...
                span { "© 2025 AZA, Inc. All rights reserved." }
              end
            end
          end
        end
