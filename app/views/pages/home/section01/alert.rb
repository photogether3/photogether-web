class Views::Pages::Home::Section01::Alert < Views::Base
  def view_template
    div(class: "w-[355px] h-[370px] absolute bottom-[124px] left-[71px]
    border border-primary z-[3] rounded-[16px] p-[32px]
    bg-[#1B1D21CC]/80 backdrop-blur-md") do
      ...
      div(class: "flex flex-col items-center gap-[10px]") do
        img(src: "/images/landing/section01/item05.png", class: "w-[101px]")
        strong(class: "text-xl") { "택스트를 추출 합니다." }
        div(class: "text-center text-lg text-[#D8D8D8]") do
          p { "업로드하신 이미지에서 텍스트를" }
          p { "자동으로 추출합니다." }
        end
        div(class: "mt-[20px] flex justify-between gap-[16px] w-full") do
          button(class: "pg-btn btn-neutral border-neutral-content flex-1") { "취소" }
          button(class: "pg-btn btn-primary flex-1") { "확인" }
        end
      end
    end
  end
